...
export default function QueuePanel({ queue, currentIndex, onClose }) {
  ...
  return (
    <>
      ...
      {/* Panel - Centered above player */}
      <div className="fixed bottom-44 left-1/2 -translate-x-1/2 w-[90%] max-w-xl bg-mosh-dark/95 backdrop-blur-xl border border-mosh-border rounded-xl shadow-2xl z-50 flex flex-col max-h-[60vh]">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-mosh-border">
          <h3 className="font-bold text-mosh-light">Queue</h3>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          {queue.length === 0 ? (
            <div className="p-8 text-center text-mosh-muted">
              ...
              <p>Queue is empty</p>
              <p className="text-xs mt-1">Play a song to start your queue</p>
            </div>
          ) : (
            queue.map((song, index) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 hover:bg-mosh-hover cursor-pointer transition ${
                  index === currentIndex ? 'bg-mosh-accent/10 border-l-2 border-mosh-accent' : ''
                }`}
              >
        ...
        {/* Footer */}
        <div className="p-3 border-t border-mosh-border flex items-center justify-between">
          <p className="text-xs text-mosh-muted">
            {queue.length} song{queue.length !== 1 ? 's' : ''} in queue
          </p>
          {currentIndex !== null && currentIndex >= 0 && (
            <p className="text-xs text-mosh-accent">
